...
**Possible future path, not yet decided on:** research (`docs/research/erpnext-email-to-document-intake.md`) confirms Frappe has a native Email Account → `append_to` mechanism — flag a doctype as `email_append_to`, point an Email Account at it, and an inbound email auto-creates a record of that doctype (populating a `sender_field`), with the original email + attachments preserved via a linked Communication. Pure configuration, no code. If Sales Order were flagged this way, a client's PO email could become a Sales Order with no manual step at all — closer to solving the "someone has to read the inbox" problem than anything considered earlier in this session. Not yet acted on: still needs a decision on whether that's wanted, plus it inherits real gotchas (threading only matches on `In-Reply-To` or a 60-day subject+sender fallback; oversized attachments silently drop past a 25MB default limit).
